...
func writeItemFile(outDir string, itemDeclarations []ItemDeclaration, itemResponses []WowheadItemResponse, spec *api.Spec) {
	...
	if spec == nil {
		file.WriteString("// +build raidsim\n\n")
	} else {
		switch *spec {
		case api.Spec_SpecElementalShaman:
			file.WriteString("// +build eleshaman\n\n")
		case api.Spec_SpecBalanceDruid:
			file.WriteString("// +build balancedruid\n\n")
		}
	}
